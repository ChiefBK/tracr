...
func readBotFile(filePath string) (*Bot, error) {
	...
	log.Debug("raw json", "json", string(rawJson))

	var bot *Bot

	err := json.Unmarshal(rawJson, &bot)

	log.Debug("botname", "name", bot.Name)
...
//	}
//
//	var action *actions.ExternalAction
//	if actionNotNull {
...
func toGOB64(bot *Bot) string {
	...
	if err != nil {
		log.Error("failed Bot encode", "module", "command", "botKey", bot.Name, "error", err)
	}
